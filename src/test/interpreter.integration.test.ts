...
// Helper functions
function executeProgram(programDef: ProgramDefinition, inputs: string[] = []): {
...
function testProgramExecution(programDef: ProgramDefinition, testName: string, inputs: string[] = []) {
	it(testName, () => {
		expect(() => {
	...
	programs.easy.forEach((program, index) => {
		...
		if (program.inputSets && program.inputSets.length > 0) {
			...
			program.inputSets.forEach((inputSet, setIndex) => {
				testProgramExecution(program, `${testName} (Input Set ${setIndex + 1})`, inputSet);
			});
...
describe("Integration Tests - Specific Program Validations", () => {
	...
	it("should handle input programs correctly", () => {
		...
		const inputProgram = programs.easy.find(p => p.inputSets && p.inputSets.length > 0);
	...
	it("should handle arithmetic operations in real programs", () => {
		...
		const arithmeticProgram = programs.easy.find(p => 
			p.code.includes('+') || p.code.includes('-') || p.code.includes('*') || p.code.includes('/')
		);
	...
	it("should handle string operations in real programs", () => {
		...
		const stringProgram = programs.easy.find(p => 
			p.code.includes('"') && p.code.includes('+')
		);
	...
	it("should handle type conversions in real programs", () => {
		...
		const conversionProgram = [...programs.easy, ...programs.medium].find(p => 
			p.code.includes('int(') || p.code.includes('str(') || p.code.includes('float(')
		);
		
		if (conversionProgram) {
			const inputs = conversionProgram.inputSets ? conversionProgram.inputSets[0] : [];
...
describe("Integration Tests - Program Output Validation", () => {
	it("should produce expected outputs for greeting program", () => {
		...
		const greetingProgram = programs.easy.find(p => 
			p.description.toLowerCase().includes('greeting') || 
			p.code.includes('Hello')
		);
		
		if (greetingProgram && greetingProgram.inputSets) {
			const result = executeProgram(greetingProgram, greetingProgram.inputSets[0]);
			expect(result.outputs.length).toBeGreaterThan(0);
			expect(result.outputs.some(output => output.includes('Hello'))).toBe(true);
		}
	});

	it("should handle calculation programs correctly", () => {
		// Find a calculation program
		const calcProgram = [...programs.easy, ...programs.medium].find(p => 
	...
	it("should handle programs with multiple outputs", () => {
		...
		const multiOutputProgram = [...programs.easy, ...programs.medium].find(p => 
			(p.code.match(/print\(/g) || []).length > 1
		);
		
		if (multiOutputProgram) {
			const inputs = multiOutputProgram.inputSets ? multiOutputProgram.inputSets[0] : [];
			const result = executeProgram(multiOutputProgram, inputs);
			
			const printCount = (multiOutputProgram.code.match(/print\(/g) || []).length;
...
describe("Integration Tests - Trace Generation", () => {
	it("should generate trace steps for all lines", () => {
		...
		const executableLines = program.code.split('\n').filter(line => 
...
describe("Integration Tests - Error Handling", () => {
	it("should handle programs with potential division by zero", () => {
		...
		const divisionPrograms = [...programs.easy, ...programs.medium, ...programs.hard].filter(p => 
			p.code.includes('/') || p.code.includes('DIV')
		);
	...
	it("should handle programs with array access", () => {
		...
		const arrayPrograms = [...programs.easy, ...programs.medium, ...programs.hard].filter(p => 
			p.code.includes('[') && p.code.includes(']')
		);
	...
	it("should handle programs with string operations", () => {
		...
		const stringPrograms = [...programs.easy, ...programs.medium, ...programs.hard].filter(p => 
			p.code.includes('"') && p.code.includes('+')
		);
