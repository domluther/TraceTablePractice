...
// Helper functions
function executeAndGetVariable(code: string, varName: string, inputs: string[] = []): any {
...
function executeAndGetVariables(code: string, inputs: string[] = []): Record<string, any> {
	const interpreter = new ASTInterpreter();
	const program: Program = { code, description: "test", inputs };
	const result = interpreter.executeProgram(code, program);
