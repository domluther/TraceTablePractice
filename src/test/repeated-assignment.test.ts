...
next x`;
		...
		const countryAssignmentSteps = trace.filter(step => 
			step.changedVariables && 'country' in step.changedVariables
		);

		console.log(`Number of times 'country' appears in changedVariables: ${countryAssignmentSteps.length}`);
...
next i`;
		...
		const valueAssignmentSteps = trace.filter(step => 
			step.changedVariables && 'value' in step.changedVariables
		);
