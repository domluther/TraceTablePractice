...
describe("Simple Program Tests", () => {
	...
	it("should run a simple text display program", () => {
		...
		const textProgram = programs.easy.find(p => 
			p.code.includes('print') && !p.code.includes('input') && p.code.includes('"')
		);
	...
	it("should run a name input program with test data", () => {
		...
		const nameProgram = programs.easy.find(p => 
			p.code.includes('input') && p.code.includes('name')
		);
	...
	it("should handle programs without loops or complex control structures", () => {
		...
		const simplePrograms = programs.easy.filter(p => 
			!p.code.includes('for ') && 
	...
	it("should handle type conversion programs", () => {
		...
		const conversionPrograms = programs.easy.filter(p => 
