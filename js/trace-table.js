...
export class TraceTable {
    ...
    markAnswer() {
        ...
        this.expectedTrace.forEach((expectedEntry, traceIndex) => {
            ...
            if (userIndex < userEntries.length && userEntries[userIndex].lineNumber === lineNum) {
                    ...
                    if (hasChanged) {
                        ...
                        if (expectedValue === undefined || expectedValue === null) {
                            if (userValue === '' || userValue === 'undefined') {
                                ...
                                feedback.push(`Line ${lineNum}, ${varName}: Expected empty/undefined, got "${userValue}"`);
                            ...
                            if (userValue === expectedValue.toString()) {
                                ...
                                feedback.push(`Line ${lineNum}, ${varName}: Expected "${expectedValue}", got "${userValue}"`);
                                inputElement.classList.add('incorrect');
                            }
                        }
                    } else {
                ...
                if (userEntry.output === expectedEntry.output) {
                    ...
                    feedback.push(`Line ${lineNum}, Output: Expected "${expectedEntry.output}", got "${userEntry.output}"`);
        ...
        while (userIndex < userEntries.length) {
            feedback.push(`Extra line ${userEntries[userIndex].lineNumber} in your trace table`);
        ...
        if (feedback.length > 0) {
            ...
            detailsDiv.innerHTML = '<strong>Details:</strong><br>' + 
                feedback.map(item => `<div class="feedback-item incorrect">${item}</div>`).join('');
