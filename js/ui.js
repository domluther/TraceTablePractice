...
export class UI {
	...
	generateSpecificProgram(difficulty, programIndex) {
		...
		if (
			this.currentProgram.inputSets &&
			this.currentProgram.inputSets.length > 0
		) {
			const randomInputSet =
				this.currentProgram.inputSets[
					Math.floor(Math.random() * this.currentProgram.inputSets.length)
				];
		...
		if (
			this.currentProgram.randomValues &&
			this.currentProgram.randomValues.length > 0
		) {
			const randomValue =
				this.currentProgram.randomValues[
					Math.floor(Math.random() * this.currentProgram.randomValues.length)
				];
	...
	displayCode(code) {
		...
		if (this.currentProgram?.inputs?.length > 0) {
	...
	shuffleInputs() {
		...
		const hasMultipleInputs = currentProgram?.inputSets?.length > 1;
	...
	updateContentVisibility() {
		...
		const hasProgramSelected =
			this.currentDifficulty && this.currentProgramIndex !== null;
	...
	loadFromURL() {
		...
		if (difficulty && programIndex !== null) {
			...
			if (
				programs[difficulty] &&
				parseInt(programIndex) < programs[difficulty].length
			) {
